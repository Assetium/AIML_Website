...
const kiddiesPortalSchema = z.object({
...
const KiddiesPortal = () => {
  ...
  const form = useForm<z.infer<typeof kiddiesPortalSchema>>({
    resolver: zodResolver(kiddiesPortalSchema),
    defaultValues: {
      telephoneNumber: "",
  ...
  const onSubmit = async (values: z.infer<typeof kiddiesPortalSchema>) => {
    setIsLoading(true);
    ...
    try {
      ...
      await router.push(
        `/open-account/kiddies-account?id=${encodeURIComponent(hash)}`
      );
    } catch (error: unknown) {
